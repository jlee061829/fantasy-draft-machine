...
- 5.4 — Mock Draft Creation / Fill Empty Slots with Bots — **NEXT**
- 5.5 — Position-Aware Bot Strategy — not started
- 5.6 — Bot Strategy Variants + Phase 5 Closeout — not started

**Phase 5 is not complete.** The data-model foundation (5.1), BEST_AVAILABLE decision logic (5.2), and server-side BOT turn orchestration (5.3) have all shipped — a BOT `LeagueMember` on the clock now gets picked for automatically, server-side, through the same authoritative transactional/broadcast path every other pick source uses. What remains missing is entirely product-accessibility: **no production path creates a BOT `LeagueMember` yet** — there is no mock-draft creation flow, no fill-empty-slots UI, and no way for a real user to ever cause a BOT to exist in a real League. Bots remain reachable only through test-only fixtures. See Milestones 5.1/5.2/5.3's own completed-work notes above for the exact boundary of what shipped at each step.
...
Current Phase 5 capabilities (Milestones 5.1–5.3):
...
- **no bot appears in any product-facing creation flow yet** — no production path creates a BOT `LeagueMember`; bots remain reachable only through test-only fixtures (`createTestBotMember`). This is Phase 5.4's job
- see Milestone 5.1's, 5.2's, and 5.3's own notes for the full boundary of what shipped, and "Known issue — League deletion blocked by Pick → LeagueMember FK ordering" below for a verified, still-open limitation Milestone 5.1 surfaced

**Phase 4 exit criteria satisfied.** All six milestones (4.1–4.6) are complete, and Phase 4 is frozen as a completed foundation the same way Phases 2 and 3 were, unless a later phase exposes a concrete defect. Phase 3's originally-scoped milestones (3.1–3.4, plus the reconnect/resync capability originally scoped as 3.5 — see note above) remain complete and frozen as well. Horizontal scalability (Redis pub/sub across multiple socket-server instances), rate limiting, structured error responses, Playwright E2E coverage, and CI remain explicitly deferred to Phase 5's own closeout (5.6) and beyond; Phase 5 itself is underway (Milestones 5.1–5.3 complete, 5.4 next).
...
- GitHub Actions CI
- any product-accessible way to create a BOT `LeagueMember` — mock-draft creation UI, fill-empty-slots UI, or any other user-facing flow (Phase 5.1 built the data model, 5.2 built BEST_AVAILABLE selection, 5.3 built server-side execution of a BOT's turn — but bots remain reachable only through test-only fixtures; this is explicitly Phase 5.4's job)
...
- a BOT-specific visual indicator/badge in the draft-room UI (a BOT pick currently renders with no `AUTO` badge, since `wasAutopick: false`, and no BOT-specific badge exists either)
...
- tighter BOT-pick pacing than "one pick per league per sweep tick" — a long all-BOT chain currently drains at up to one pick per sweep interval (default 2000ms) per league; this is an intentional correctness-first starting point (see Milestone 5.3's notes), and may be revisited later if mock-draft UX demands faster pacing, but any such change must remain row-lock-based, not introduce a new correctness mechanism
...
**Bot Managers + Mock Drafts — superseded by Phase 5 (Milestones 5.1–5.3 complete as of this update).** This subsection is preserved for historical/design-rationale context; it is no longer "unscoped" — see "Build phases" for the committed 5.1–5.6 milestone list, "Settled decisions" for what 5.1–5.3 actually locked in, and the Milestone 5.1/5.2/5.3 completed-work notes above for exactly what shipped. The bullets below predate implementation and are evaluated one at a time as each assumption turned out to hold or need adjustment:
...
Phase 5 is not complete; Milestones 5.1–5.3 have shipped.
