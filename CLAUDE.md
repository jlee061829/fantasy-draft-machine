...
- Phase 5 Milestone 5.5 — Position-Aware Bot Strategy:
  - added `selectPositionAwareBotPlayerId(tx, { draftId, leagueMemberId, scoringFormat, rosterSize, currentPickNumber, teamCount })` in new `packages/database/src/drafts/position-aware-selection.ts` — the BOT-only counterpart to `selectBestAvailablePlayerId`, deliberately a *separate* function rather than a modification of it
  - **strategy split (deliberate, not incidental):** `processExpiredDraftTurn` (human timer-expiry autopick) keeps calling `selectBestAvailablePlayerId`, completely unmodified — a human who misses their deadline still gets pure BEST_AVAILABLE. Only `processBotDraftTurn` calls the new selector. A human's missed deadline and a bot's own intentional pick remain different events; `wasAutopick`'s existing meaning ("timer expired") does not start silently reflecting strategy no human chose
  - the new selector is Prisma-dependent, lives in `packages/database`, and is **not** exported from `packages/database/src/index.ts` — it is an implementation detail of BOT turn processing specifically, consumed only by `bot-turn.ts`; `processBotDraftTurn` remains the one safe, public, high-level entry point `apps/socket-server` is allowed to call. It joins `lockDraftForLeague`/`applyPick` in the internal-only group, not `selectBestAvailablePlayerId` in the public group
  - read-only, exactly like `selectBestAvailablePlayerId`: never writes a Pick, never touches `Draft`, owns no part of the transactional correctness boundary itself; called inside `processBotDraftTurn`'s existing locked transaction, never outside it
  - candidate eligibility is unchanged from Phase 5.2: undrafted (in this `draftId`), rostered (`Player.nflTeam IS NOT NULL`) players only — the pool was not broadened or narrowed
  - strategy model: `adjustedScore = baseValue + positionPenalty`, lower wins, computed over the **full** eligible candidate pool (no bounded top-N window) scored in TypeScript — the current rostered pool is ~1,068 players, small enough that a bounded window would risk excluding the true best-fit candidate for no real performance benefit
  - `baseValue` is the candidate's own ADP (tier 1, when a non-null ADP row exists for the league's `scoringFormat`) or `searchRank` (tier 2, fallback) — Phase 5.2's two-tier precedence is preserved exactly: every tier-1 candidate ranks ahead of every tier-2 candidate regardless of position penalties on either side; penalties only reorder candidates *within* a tier. This is a deliberately conservative compatibility decision, not an attempt to build a single cross-tier ADP/searchRank unit conversion (which would itself be an unjustified magic number)
  - roster position counts are derived only from `Pick.draftId = <this draft> AND Pick.leagueMemberId = <this BOT's own LeagueMember.id>`, joined to `Player.position` — never `userId`; other participants' Picks structurally cannot affect this BOT's own penalties
  - positional penalty bands (soft, additive, never a hard exclusion — a sufficiently large raw-value gap can still overcome any of them):
    - `QB`: 0 owned → `+0`, 1 owned → `+18`, 2+ owned → `+60`
    - `TE`: 0 owned → `+0`, 1 owned → `+15`, 2+ owned → `+55`
    - `RB`: 0–2 owned → `+0`, 3 owned → `+5`, 4+ owned → `+12`
    - `WR`: 0–3 owned → `+0`, 4 owned → `+5`, 5+ owned → `+12`
  - **K/DEF timing rule (explicit product decision, added after the initial 5.5 pass):** K and DEF are strongly discouraged before the final 3 rounds, deliberately overriding raw ADP rather than merely following it — `roundNumber = ceil(currentPickNumber / teamCount)`, `lateWindowStart = max(1, rosterSize - 2)` (round 13 of 15 for the current product default), `isLateWindow = roundNumber >= lateWindowStart`. Timing penalty: `+100` before the late window, `+0` during it. A separate, additive duplicate-ownership penalty (`0 owned → +0`, `1 owned → +40`, `2+ owned → +80`) applies identically inside and outside the late window, identically for K and DEF
  - the K/DEF timing rule is still soft, not an absolute exclusion — no `if round < X: exclude K/DEF` branch exists; an extreme enough candidate-pool situation can still make an early K/DEF the correct score-based pick, verified by a dedicated test
  - initial (pre-timing-rule) 5.5 finding, superseded: real seeded ADP alone already placed K/DEF reasonably late in the common case, so K/DEF initially shipped with **no** penalty at all. The explicit product decision above replaced that with the stronger, deliberate `+100`/duplicate-band rule described here — this is the current, final behavior
  - deterministic tier-1/tier-2 comparators, with null `searchRank` handled by explicit branching rather than a numeric sentinel (a sentinel large enough to guarantee tier ordering would make position penalties numerically negligible against it for exactly the null-`searchRank` candidates — historically, all `DEF` rows — the penalty is meant to affect): tier 1 sorts `adjustedScore ASC → raw ADP ASC → searchRank(non-null before null) → searchRank ASC → id ASC`; tier 2 (real `searchRank`) sorts `adjustedScore ASC → raw searchRank ASC → id ASC`, with null-`searchRank` tier-2 candidates sorting after all of those and among themselves by `penalty ASC → id ASC`
  - fully deterministic: same committed Draft/Pick state, same BOT roster, same scoring format, same available Players → same selected `playerId`, every time; no randomness, weighting, personality, or difficulty exists anywhere in the selector
  - `processBotDraftTurn`'s only change: the selector call site now passes `leagueMemberId: draft.currentMemberId`, `rosterSize: league.rosterSize`, `currentPickNumber: draft.currentPickNumber`, `teamCount: league.teamCount` in addition to `draftId`/`scoringFormat`. Everything else — Draft-row lock, participant-type validation, `applyPick`, `leagueMemberId` attribution, `wasAutopick: false`, turn advancement, `BotTurnOutcome` shape, `BotPickExhaustedError` on exhaustion — is byte-for-byte unchanged
  - no changes anywhere to: Draft-row locking, `applyPick`, turn progression, the BOT sweep, socket broadcast behavior, retry/error handling, the Socket.IO protocol, `apps/web`, `packages/shared`, or the Prisma schema
  - no lineup-slot schema, no minimum-position requirement (QB/TE/K/DEF), no FLEX logic, and no exact-roster-composition guarantee were introduced — the strategy is a drafting heuristic, not a roster validator; a BOT may still finish a draft without a K or DEF in some edge/synthetic situations, which is accepted for 5.5 and is not treated as a correctness bug
  - no `botStrategy` schema field, no strategy enum/dispatch layer, no strategy variants, no personality/difficulty/randomness, and no UI/protocol changes of any kind were introduced — bots simply choose better players through the unchanged existing system; see "Bot position-aware strategy conventions" below for full detail
- Milestone 5.5 verification:
  - `packages/database`: 162/162 tests passing (up from 121) — focused coverage in new `position-aware-selection.test.ts` includes: empty-roster baseline, QB moderate/strong penalty, a nearby non-QB beating an over-stocked QB, a sufficiently large value gap still winning QB2 over the penalty, TE moderate/strong penalty, RB/WR light depth penalty and value override, current-membership-only ownership scoping (another member's QB-heavy roster does not affect the BOT under test; ownership keyed by `leagueMemberId`, never `userId`), scoring-format sensitivity, drafted-player exclusion, cross-Draft eligibility, tier-1-always-outranks-tier-2 even against a heavy penalty, tier-2 real-vs-null-`searchRank` ordering, deterministic tie-breaks, exhaustion, repeated-call determinism, the K/DEF round-12/round-13 timing boundary (both K and DEF), the K/DEF soft-override case, all five exact K/DEF duplicate-penalty bands (proven to the exact integer via bracketing competitor ADPs, for both K and DEF), and a `rosterSize=8` short-roster late-window boundary case
  - `bot-turn.test.ts` gained a "position-aware selection (wiring)" test proving `processBotDraftTurn` actually uses the new selector (a BOT owning multiple QBs picks a needed WR over an available QB, with the Pick still applied through the normal path, `wasAutopick: false`)
  - `autopick.test.ts` gained a human/BOT strategy-separation regression test: an identical QB-heavy-roster shape is constructed for a HUMAN timeout, and `processExpiredDraftTurn` is proven to still select the raw best-available QB — the exact scenario where the parallel BOT test picks the WR instead — directly proving the two selectors diverge only for bots, not merely asserting it from reading the code
  - a representative full-draft simulation (`bot-strategy-simulation.test.ts`, 12 teams, 15 rounds, SNAKE, all-BOT, 180 picks, real Postgres, a deterministic synthetic player pool shaped to mirror real seeded PPR ADP ordering) reached `COMPLETE` with exactly 180 Picks, no duplicate Players, every BOT receiving exactly 15 Picks, and contiguous `pickNumber`s 1–180; per-team QB counts 1–2, TE counts 1–2, RB counts 4–7, WR counts 5–8, no team with 2+ K or 2+ DEF; earliest K pick 149 (round 13), earliest DEF pick 145 (round 13); K-count distribution across the 12 teams: 7 with 0, 5 with 1, 0 with 2+; DEF-count distribution: 4 with 0, 8 with 1, 0 with 2+
  - a separate, smaller determinism simulation (4 teams, 15 rounds, 60 picks, run twice from fresh identical initial state) produced an identical pick sequence when compared by the stable tuple `(pickNumber, draftSlot, playerFullName)` — deliberately not compared by DB-generated `LeagueMember`/`Draft`/`Player` ids, which differ across runs by construction
  - manual/service-level verification against the real dev database (`fantasy_draft`, real seeded Player/ADP data, 11 real BOT participants + 1 HUMAN control participant using the one real OAuth identity) confirmed, by directly querying which participant made each K/DEF pick: **every BOT K/DEF pick occurred at round 13 or later, with zero exceptions** — earliest BOT K at pick 147 (round 13), earliest BOT DEF at pick 146 (round 13); rounds 1–12 for every BOT were exclusively QB/RB/WR/TE; 10 of 11 BOTs finished with exactly 1 K + 1 DEF, 1 BOT finished with 1 K and 0 DEF, and no BOT accumulated duplicate K/DEF depth; QB/TE stayed in the same plausible 1–2 range as the synthetic simulation; RB/WR retained healthy depth
  - the HUMAN control participant (deliberately left on unmodified `selectBestAvailablePlayerId` semantics, per the human/BOT separation decision) accumulated 4 DEF and picked its first DEF far earlier than any BOT — a real, organically-observed demonstration of exactly the degenerate pattern position-aware BOT strategy exists to avoid, occurring next to 11 BOTs that avoided it entirely against the identical candidate pool. This is expected and correct, not a defect: human timer-autopick intentionally retains its simpler, unmodified BEST_AVAILABLE semantics
  - workspace-wide typecheck and build pass; `apps/socket-server` (37/37) and `apps/web` (364/364) tests pass unaffected, confirming no source changes were needed outside `packages/database`
...
- Phase 5.5 settled decisions — Position-Aware Bot Strategy:
  - `selectPositionAwareBotPlayerId` (`packages/database/src/drafts/position-aware-selection.ts`) is a **new, separate** selector, not a modification of `selectBestAvailablePlayerId` — human timer-expiry autopick (`processExpiredDraftTurn`) keeps calling the original, completely unmodified selector; only `processBotDraftTurn` calls the new one. This is deliberate: a human's missed deadline and a bot's own intentional pick are different events, and `wasAutopick`'s pre-existing meaning ("timer expired") must not silently start reflecting roster-building strategy no human chose
  - the new selector is Prisma-dependent and lives in `packages/database`, matching `selectBestAvailablePlayerId`'s package, but is **deliberately not exported** from `packages/database/src/index.ts` — unlike `selectBestAvailablePlayerId` (read-only and safe to expose), it is an implementation detail of BOT turn processing specifically, reachable only through `processBotDraftTurn`. It joins `lockDraftForLeague`/`applyPick` in the package's internal-only group
  - candidate eligibility is unchanged from Phase 5.2: undrafted-in-this-`draftId`, rostered (`Player.nflTeam IS NOT NULL`) Players only; the pool was neither broadened nor narrowed
  - scoring model: `adjustedScore = baseValue + positionPenalty` (lower wins), computed by loading the **full** eligible candidate pool (no bounded top-N window) and scoring entirely in TypeScript — the ~1,068-player rostered pool is small enough that a bounded window would risk silently excluding the true best-fit candidate for no meaningful performance benefit
  - Phase 5.2's two-tier precedence (tier 1 = has a usable ADP for the league's `scoringFormat`; tier 2 = fallback to `searchRank`) is preserved exactly: every tier-1 candidate outranks every tier-2 candidate regardless of position penalties on either side; penalties only reorder candidates *within* a tier. This is a deliberately conservative compatibility decision, avoiding an invented cross-tier ADP/searchRank unit-conversion constant
  - null `searchRank` inside tier 2 is handled by explicit branching (real-`searchRank` tier-2 candidates always sort before null-`searchRank` ones, which then sort among themselves by `penalty ASC, id ASC`), not by a numeric sentinel like `Number.MAX_SAFE_INTEGER` — a sentinel that large would make position penalties numerically negligible against it for exactly the null-`searchRank` candidates (historically, all `DEF` rows) the penalty is meant to affect
  - roster position counts are derived exclusively from `Pick.draftId = <this draft> AND Pick.leagueMemberId = <this BOT's own LeagueMember.id>` joined to `Player.position` — never `userId`; another participant's Picks are structurally excluded from this query and cannot affect the current BOT's penalties
  - positional penalty bands are soft/additive, never a hard exclusion — `if round < X: exclude K/DEF`-style absolute rules were deliberately not written anywhere; a sufficiently large raw-value gap can always overcome a penalty, by construction, for every position including K/DEF
  - final bands: `QB` (0/+0, 1/+18, 2+/+60), `TE` (0/+0, 1/+15, 2+/+55), `RB` (0-2/+0, 3/+5, 4+/+12), `WR` (0-3/+0, 4/+5, 5+/+12) — QB/TE bands start at the *second* owned player (a first QB/TE draws no penalty), RB/WR bands are deliberately much lighter, matching real rosters commonly carrying several of each
  - **K/DEF timing rule is an explicit product decision, layered on top of the above** (added in a follow-up revision within 5.5, after an initial no-penalty pass found real seeded ADP alone already placed K/DEF reasonably late in the common case — see "Bot position-aware strategy conventions" for that superseded finding): K/DEF should be strongly discouraged, not merely mildly deprioritized, before the final 3 rounds, *even overriding an attractively low raw ADP* — `roundNumber = ceil(currentPickNumber / teamCount)`, `lateWindowStart = max(1, rosterSize - 2)`, `isLateWindow = roundNumber >= lateWindowStart`; timing penalty is `+100` before the late window, `+0` during it, identical for K and DEF; an independent duplicate-ownership penalty (`0/+40/+80` by owned count) applies on top, identically inside and outside the late window
  - the selector's public-facing conceptual signature is `selectPositionAwareBotPlayerId(tx, { draftId, leagueMemberId, scoringFormat, rosterSize, currentPickNumber, teamCount })`, all six parameters required — `rosterSize`/`currentPickNumber`/`teamCount` exist solely to drive the K/DEF timing rule's dynamic round calculation, using the League's own `rosterSize` (not a hardcoded `15`), so historical non-15 leagues and internal test fixtures (e.g. `rosterSize=8`) compute a correctly-scaled late window
  - selection remains inside `processBotDraftTurn`'s existing locked transaction — no query moved outside the lock; `processBotDraftTurn`'s Draft-row lock, participant-type re-validation, `applyPick` call, `leagueMemberId`/`wasAutopick: false` attribution, turn-advancement logic, `BotTurnOutcome` shape, and `BotPickExhaustedError`-on-exhaustion behavior are all unchanged — only the selector call site itself gained the three new parameters
  - no `botStrategy` schema field, strategy enum, or strategy-dispatch layer was introduced — 5.5 ships exactly one BOT strategy, applied uniformly to every BOT; a second, genuinely different policy (Phase 5.6) is what would first justify that abstraction
...
4. a genuine BOT turn reads League config, then calls `selectPositionAwareBotPlayerId` (Phase 5.5 — see "Bot position-aware strategy conventions") inside the same transaction, then applies the result through the identical `applyPick(...)` used by every other pick source, with `leagueMemberId` = the BOT's own `LeagueMember.id` and `wasAutopick: false`
...
**Player selection — two distinct selectors as of Phase 5.5 (see "Bot position-aware strategy conventions" for full detail):**

- `selectBestAvailablePlayerId` (extracted in Phase 5.2, unchanged by 5.3 or 5.5): lives in `packages/database/src/drafts/player-selection.ts`, exported through `@fdm/database`'s public entry point. Signature: `selectBestAvailablePlayerId(tx: Prisma.TransactionClient, { draftId, scoringFormat }): Promise<string | null>`. Read-only — never writes a Pick, never touches `Draft`, owns no part of the transactional correctness boundary itself. Still used exclusively by `processExpiredDraftTurn` (human timer-expiry autopick), which retains pure BEST_AVAILABLE behavior
- `selectPositionAwareBotPlayerId` (Phase 5.5, new): lives in `packages/database/src/drafts/position-aware-selection.ts`, **not** exported through `@fdm/database`'s public entry point — an internal implementation detail reachable only through `processBotDraftTurn`. Used exclusively by `processBotDraftTurn`; layers per-position ownership penalties (and an explicit K/DEF timing rule) on top of the same ADP/searchRank tier structure `selectBestAvailablePlayerId` uses
- no `BotDraftStrategy` enum or strategy-dispatch layer exists for either selector — each is one function's fixed behavior, not a selectable value, until a second, genuinely different BOT policy exists (Phase 5.6)

`selectBestAvailablePlayerId` is scoped to Players not yet drafted in the supplied `draftId` (a Player drafted only in a *different* Draft remains eligible — draft pools are per-Draft, not global) and to `Player.nflTeam IS NOT NULL` (rostered players only):
...
- no roster-position awareness — that's what `selectPositionAwareBotPlayerId` (BOT-only) adds on top of the identical eligibility rules and tier structure; see "Bot position-aware strategy conventions"
- returns `null` when no eligible undrafted rostered Player exists in either tier; the selector itself never throws — each caller defines its own exhaustion semantics: `processExpiredDraftTurn` converts `null` into `AutopickExhaustedError`; `processBotDraftTurn` (via `selectPositionAwareBotPlayerId`, which shares this same null-on-exhaustion contract) converts it into the distinct `BotPickExhaustedError` (see below)

**`BotPickExhaustedError` (Phase 5.3, still thrown by the same call site as of 5.5):** thrown by `processBotDraftTurn` when its selector — `selectBestAvailablePlayerId` before Phase 5.5, `selectPositionAwareBotPlayerId` as of Phase 5.5 — returns `null` for a BOT's turn — the same underlying condition `AutopickExhaustedError` represents (the seeded rostered Player pool is smaller than `teamCount * rosterSize`), kept as a distinct class so BOT-pool exhaustion can be logged/handled separately from human-autopick-pool exhaustion if that distinction is ever useful. Thrown inside the transaction, so it rolls back completely: no Pick written, `currentPickNumber`/`currentMemberId` unchanged, Draft stays `ACTIVE` with the same BOT current participant. `apps/socket-server`'s BOT-phase catch block logs it and continues to the next league in the batch, exactly mirroring `AutopickExhaustedError`'s existing handling; the affected Draft is retried identically on every subsequent tick until the underlying seed/roster-size mismatch is corrected. No product-facing recovery UI exists.
...
### Bot position-aware strategy conventions

Phase 5.5 conventions. This is what turns BOT turns from pure BEST_AVAILABLE (Phase 5.3) into roster-aware selection, while leaving human timer-autopick completely untouched.

**Strategy separation (the single most important rule here):**

```text
HUMAN timeout:
  processExpiredDraftTurn → selectBestAvailablePlayerId   (unchanged, pure BEST_AVAILABLE)

BOT turn:
  processBotDraftTurn → selectPositionAwareBotPlayerId    (Phase 5.5, roster-aware)
```

These are two separate functions, not one function branching on participant type. A human who misses their deadline gets exactly the same BEST_AVAILABLE selection they always did; only an actual BOT turn uses the new strategy. `wasAutopick` keeps its pre-5.5 meaning exactly (`true` only for a human's missed-deadline autopick; `false` for a BOT's own intentional pick, unchanged since Phase 5.3).

**Selector location and visibility:**

`packages/database/src/drafts/position-aware-selection.ts` — Prisma-dependent, like `player-selection.ts`, so it lives in `packages/database`, not `packages/shared`. Unlike `selectBestAvailablePlayerId`, it is **not** exported from `packages/database/src/index.ts` — it is an internal implementation detail of BOT turn processing, consumed only by `bot-turn.ts`. `processBotDraftTurn` remains the one safe, public, high-level entry point `apps/socket-server` is allowed to call; the strategy selector joins `lockDraftForLeague`/`applyPick` in the internal-only group rather than `selectBestAvailablePlayerId` in the public group.
...
`rosterSize`/`currentPickNumber`/`teamCount` exist solely to drive the K/DEF timing rule's dynamic round calculation below — `leagueMemberId` is the BOT's own `LeagueMember.id`. No schema change was required to add these; they're already available on the locked `Draft`/`League` rows `processBotDraftTurn` reads.

**Roster ownership counting:**

```text
Pick.draftId = <this draft>
AND
Pick.leagueMemberId = <this BOT's own LeagueMember.id>
```

joined to `Player.position`, counted in TypeScript (a BOT's roster is bounded by `rosterSize`, so this is trivially small). Never `userId` — a BOT has none. Other participants' Picks are structurally excluded from this query and cannot affect the current BOT's own positional penalties; verified directly with a dedicated test (a second, heavily QB-stocked member's Picks do not affect the BOT under test).

**Candidate eligibility (unchanged from Phase 5.2):**

```text
Player.nflTeam IS NOT NULL
AND
Player has not already been Picked in this same Draft
```

The pool was neither broadened nor narrowed for 5.5. Scoring-format-specific `PlayerAdp` remains the primary value input.

**Full-pool scoring, not a bounded window:** the selector loads (1) the BOT's own current position counts, then (2) the entire eligible undrafted rostered Player pool (currently ~1,068 Players at the upper bound, shrinking as any given draft progresses) in one query, then scores every candidate in TypeScript and picks the best deterministically. No arbitrary top-N candidate window is used, and no positional-penalty logic is expressed in SQL — a bounded window risked excluding the true best-fit candidate for a BOT's specific needs just outside an arbitrary cutoff, for no real performance benefit at this pool size.

**Tier precedence (preserved from Phase 5.2, a deliberate conservative compatibility decision):**

- **Tier 1** — the candidate has a non-null `PlayerAdp.adp` row for the league's `scoringFormat`
- **Tier 2** — the candidate has no usable matching ADP

Every Tier-1 candidate ranks ahead of every Tier-2 candidate, unconditionally — position penalties only reorder candidates *within* a tier, never across tiers. This avoids inventing a cross-tier ADP/searchRank unit-conversion constant, which would itself be an unjustified magic number.

**Deterministic Tier-1 ordering** (lower `adjustedScore` wins):

```text
1. ADP + positionPenalty ASC
2. raw ADP ASC
3. searchRank non-null before null
4. searchRank ASC (when non-null)
5. Player.id ASC
```

**Deterministic Tier-2 ordering** — null `searchRank` is handled by explicit branching, never a numeric sentinel like `Number.MAX_SAFE_INTEGER` (a sentinel that large would make position penalties numerically negligible against it for exactly the null-`searchRank` candidates — historically, every `DEF` row — the penalty is meant to affect):

For Tier-2 candidates with a real `searchRank`:

```text
1. searchRank + positionPenalty ASC
2. raw searchRank ASC
3. Player.id ASC
```

Candidates with a null `searchRank` sort after every Tier-2 candidate with a real `searchRank`. Among themselves:

```text
1. positionPenalty ASC
2. Player.id ASC
```

**Positional penalty bands** — soft and additive everywhere, never a hard exclusion; a sufficiently large raw-value gap can always overcome any of them, for every position including K/DEF. No `if round < X: exclude K/DEF`-style absolute rule exists anywhere in the selector.
...
QB/TE bands start at the *second* owned player (a first QB/TE draws no penalty — a single early QB/TE is completely ordinary). RB/WR bands are deliberately much lighter, since real rosters commonly carry several of each; light enough that they rarely override genuine value on their own, while still nudging a BOT toward other needs once RB/WR depth is already comfortable.

**K/DEF timing rule — an explicit product decision, distinct from the ownership-based bands above.** An initial 5.5 pass shipped K/DEF with *no* penalty at all, on the finding that real seeded ADP already placed them reasonably late in the common case (a 12-team league's ADP-covered pool never runs out before its draft completes, so K/DEF's own naturally-late ADP governed their timing without any help). That finding held for raw ADP, but was explicitly superseded by a stronger product requirement: K and DEF should be *strongly discouraged*, not merely mildly deprioritized, before the final 3 rounds — even overriding an attractively low raw ADP.
...
For the current product's fixed 15-round default, `lateWindowStart = 13` — rounds 1–12 are early/mid, rounds 13–15 are late. The formula is driven by the League's own `rosterSize`, never a hardcoded `15`, so a historical non-15 league (or an internal test fixture, e.g. `rosterSize=8` → `lateWindowStart=6`) computes a correctly-scaled late window with no special-casing.

Timing penalty, identical for K and DEF:
...
A separate, additive duplicate-ownership penalty applies on top, identically inside and outside the late window, identically for K and DEF:
...
Worked examples: Round 8 (before the late window), a BOT's first DEF → `+100`. Round 8, a BOT's second DEF → `+100 + 40 = +140`. Round 14 (late window), first DEF → `+0`. Round 14, second DEF → `+0 + 40 = +40`. Same arithmetic for K.

This remains soft, not exclusionary — verified by a dedicated test where an extreme enough candidate-pool situation still makes an early K the correct score-based pick despite the `+100` penalty.

**Strategy philosophy:** ADP represents player value; positional penalties represent draft/roster behavior. A BOT is therefore allowed to intentionally deviate from raw ADP when roster construction makes a nearby alternative more sensible — this is the literal mechanism behind "prefer the best available player, unless current roster construction makes another nearby player meaningfully more sensible." The system does not attempt to produce an optimal or expert-level fantasy draft AI; it is a plausibility heuristic, not a competitive strategy engine.

**No lineup enforcement.** Phase 5.5 introduced none of: lineup slots, a minimum QB requirement, a minimum TE requirement, a guaranteed K, a guaranteed DEF, FLEX logic, or any exact roster-composition requirement. The strategy is heuristic, not a roster validator. A BOT may still finish a draft without a K or DEF in some edge/synthetic situations — observed directly in both the representative simulation and the real-dev-DB verification below — and this is accepted for 5.5, not framed as a correctness bug. Automated tests were written to assert only invariants the strategy actually guarantees (no 2+ K, no 2+ DEF, no clearly degenerate QB/TE stockpiling, meaningful RB/WR depth), never a minimum-position count it does not.

**Representative simulation** (12 teams, 15 rounds, SNAKE, all-BOT, 180 picks, real Postgres, a deterministic synthetic player pool shaped to mirror real seeded PPR ADP ordering): Draft reached `COMPLETE`; exactly 180 Picks; no duplicate Players; every BOT received exactly 15 Picks; `pickNumber`s contiguous 1–180. Per-team QB counts 1–2, TE counts 1–2, RB counts 4–7, WR counts 5–8; no team with 2+ K or 2+ DEF. Earliest K: pick 149 (round 13). Earliest DEF: pick 145 (round 13). K-count distribution across the 12 teams: 7 with 0, 5 with 1, 0 with 2+. DEF-count distribution: 4 with 0, 8 with 1, 0 with 2+.

**Real-dev-DB verification** (real seeded Player/ADP data, 11 real BOT participants + 1 HUMAN control using the one real dev OAuth identity, 12-team/15-round SNAKE): directly querying which participant made each K/DEF pick confirmed **every BOT K/DEF pick occurred at round 13 or later, with zero exceptions** — earliest BOT K at pick 147 (round 13), earliest BOT DEF at pick 146 (round 13); rounds 1–12 for every BOT were exclusively QB/RB/WR/TE. 10 of 11 BOTs finished with exactly 1 K + 1 DEF; 1 BOT finished with 1 K and 0 DEF; no BOT accumulated duplicate K/DEF depth. QB/TE stayed in the same plausible 1–2 range as the simulation; RB/WR retained healthy depth.

**Human-control finding:** the HUMAN participant in that same real-dev-DB run — deliberately left on unmodified `selectBestAvailablePlayerId` semantics, since this strategy is BOT-only — accumulated 4 DEF and picked its first DEF far earlier than any BOT. A pure BEST_AVAILABLE participant in the same player pool can still accumulate unrealistic positional depth (many QBs, multiple defenses); the position-aware BOT strategy materially reduces this. This is expected and correct, demonstrating the intended strategy separation — not a sign that human timer-autopick is defective. Human timer-autopick intentionally retains its simpler, unmodified semantics.

**Determinism:** the selector is a pure function of committed Draft/Pick state, the requesting BOT's own roster, the league's scoring format, and the currently available Player pool — no randomness, weighting, personality, or difficulty exists anywhere in it. Verified at the single-call level (repeated calls against unchanged state return the same `playerId`) and at full-draft scale (two independent runs from identical initial state produce an identical pick sequence when compared by the stable tuple `pickNumber`/`draftSlot`/player identity — deliberately never by DB-generated `LeagueMember`/`Draft`/`Player` ids, which differ across runs by construction even when every other input is identical).

**Transaction/correctness behavior — unchanged.** The new selector is invoked inside `processBotDraftTurn`'s existing sequence — Draft-row lock → participant-type re-validation → selection → `applyPick` → commit — with no query moved outside that transaction. Nothing about the Draft lock, `applyPick`, turn progression, `BotTurnOutcome`'s shape, the BOT sweep, socket broadcast behavior, or retry/error handling changed; BOT Picks still persist `wasAutopick: false`.

**No UI/protocol changes.** Phase 5.5 required no changes to `apps/web`, `apps/socket-server`, `packages/shared`, the Socket.IO protocol, the mock-draft UI, `DraftBoard`, `TeamRosterPanel`, or the pre-draft flow — bots simply choose better players through the existing system end to end.
